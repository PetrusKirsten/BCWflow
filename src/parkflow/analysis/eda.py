...
def _coerce_bool(series: pd.Series, true_values: tuple[str, ...] = ("true", "1", "yes")) -> pd.Series:
    ...
    return series.astype(str).str.strip().str.lower().isin(true_values).astype("boolean")
...
def prepare_queue_dataset(df: pd.DataFrame, config: EDAConfig | None = None) -> pd.DataFrame:
    ...
    The raw/processed dataset can evolve as the project grows. This helper keeps
    EDA code stable by coercing key columns, adding missing time features, and
    creating a few safe derived variables.
    """
    ...
    out["has_positive_wait"] = out["wait_time"].fillna(0) > 0
...
def filter_queue_data(
    ...
    if min_wait_time is not None and "wait_time" in out.columns:
        out = out[out["wait_time"].fillna(0) >= min_wait_time]

    return out


def build_attraction_summary(df: pd.DataFrame, only_open: bool = True) -> pd.DataFrame:
...
def build_hourly_summary(df: pd.DataFrame, only_open: bool = True) -> pd.DataFrame:
...
def build_heatmap_matrix(df: pd.DataFrame, metric: str = "mean_wait", only_open: bool = True) -> pd.DataFrame:
...
def build_time_series_summary(df: pd.DataFrame, ride_name: str | None = None, only_open: bool = True) -> pd.DataFrame:
    ...
    ts = (
        ...
        .agg(mean_wait=("wait_time", "mean"), max_wait=("wait_time", "max"), observations=("wait_time", "size"))
...
def build_weather_summary(df: pd.DataFrame, only_open: bool = True) -> pd.DataFrame:
    ...
    if "rain_flag" in data.columns:
        rain = (
            ...
            .agg(observations=("wait_time", "size"), mean_wait=("wait_time", "mean"), p90_wait=("wait_time", lambda s: s.quantile(0.90)))
    ...
    if "temperature_2m" in data.columns and data["temperature_2m"].notna().sum() >= 3:
        temp = data.copy()
        try:
            temp["temperature_bucket"] = pd.qcut(temp["temperature_2m"], q=min(4, temp["temperature_2m"].nunique()), duplicates="drop")
            temp_summary = (
                temp.groupby("temperature_bucket", as_index=False, observed=True)
                .agg(
                    observations=("wait_time", "size"),
                    mean_wait=("wait_time", "mean"),
                    p90_wait=("wait_time", lambda s: s.quantile(0.90)),
                )
...
def build_initial_insights(df: pd.DataFrame) -> list[str]:
    ...
    data = prepare_queue_dataset(df)
    summary = build_attraction_summary(data, only_open=True)
    hourly = build_hourly_summary(data, only_open=True)
    insights: list[str] = []
    ...
    if not summary.empty:
        top = summary.iloc[0]
        insights.append(
            f"Highest current queue pressure by p90 is {top['ride_name']} with p90 ≈ {top['p90_wait']:.1f} min across {int(top['observations'])} observation(s)."
        )
    ...
    if "precipitation" in data.columns and data["precipitation"].notna().any():
        insights.append("Weather variables are already linked to queue records, so the project is ready for exploratory weather comparisons as coverage grows.")
